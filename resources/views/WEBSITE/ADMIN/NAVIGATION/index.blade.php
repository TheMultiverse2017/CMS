...
@section('content')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 text-end">
                            <button type="button" class="btn btn-primary col mx-2" data-bs-toggle="modal"
                                data-bs-target="#createNavigation" style="width:20em">
                                Create Navigation
                            </button>
                        </div>
                    </div>
    ...
    <!-- Modal Start-->
    <div class="modal fade" id="createNavigation" tabindex="-1" aria-labelledby="createLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="createLabel">Create Navigation</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form method="POST" action="{{ route('navigation.save') }}" enctype="multipart/form-data">
                        @csrf
                        @method('POST')
                        <div class="row">
                            <div class="col-md-12 py-2">
                                <label for="menu" class="form-label">Menu</label>
                                <input class="form-control" multiple name="menu" type="text" id="menu">
                                @error('menu')
                            ...
                            <div class="row py-2">
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        ...
                                        <input class="form-control" multiple name="submenu" type="text" id="submenu">
                                        @error('submenu')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.btnActivate', function() {
                ...
                route = "{{ route('navigation.status', ['status' => 'enable', 'id' => '__ID__']) }}"
                    .replace(
                        '__ID__', formData);
                type = 'POST';
            ...
            $(document).on('click', '.btnDeActivate', function() {
                formData = $(this).data('id');
                route = "{{ route('navigation.status', ['status' => 'disable', 'id' => '__ID__']) }}"
                    .replace(
                        '__ID__', formData);
                type = 'POST';
                ajaxForm(fromID = null, type, route, formData, table, request = 'button');
            ...
            $(document).on('click', '.btnDelete', function() {
                formData = $(this).data('id');
                route = "{{ route('navigation.delete', ['id' => '__ID__']) }}".replace('__ID__', formData);
                type = 'DELETE';
                ajaxForm(fromID = null, type, route, formData, table, request = 'button');
            ...
            $(document).on('click', '.btnEditModal', function() {
                ...
                formData = $(this).data('id');
                route = "{{ route('navigation.edit', ['id' => '__ID__']) }}".replace('__ID__', formData);
                type = 'GET';
                modal = true;
                getMenus();
                editModalData = `
                <form id="editForm" enctype="multipart/form-data">
                        <div class="row">
                                <div class="col-md-12 py-2">
                                    <label for="menu" class="form-label">Menu</label>
                                        <select required class="form-select" aria-label="Default select example" name="menudropdown" id="menudropdown">
                                        <option value="">Open this select menu</option>

                                        </select>
                                        <input class="form-control menu" multiple name="menu" type="text" id="menu">
                                    @error('menu')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                ...
                ajaxForm(fromID = '#btnEditModal #editFormDiv #editForm', type, route, formData, table =
                    null, request = 'modal', requestFrom = 'navigationEditModal');
            ...
            $(document).on('click', '#updateBtn', function() {
                ...
                const route = "{{ route('navigation.update', ['id' => '__ID__']) }}".replace('__ID__', id);
                const type = 'POST';
