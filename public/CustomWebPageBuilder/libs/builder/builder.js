...
Vvveb.FileManager = {
	...
	renamePage: function(element, e, duplicate = false) {
        ...
        if (newfile) {
            // Extract filename only (without the full path)
            let oldFileName = page.file.split('/').pop();
            ...
            fetch(renameUrl, {
                ...
                body: JSON.stringify({ file: oldFileName, newfile: newFileName, duplicate })
            })
            .then(response => response.json())
            .then(data => {
                let bg = data.success ? "bg-success" : "bg-danger";
                displayToast(bg, "Rename", data.message);

                if (data.success) {
                    let baseName = newFileName.replace('.html', '');
                    let newName = friendlyName(newFileName.replace('.html', ''));

                    _self.pages[page.page]["file"] = newFileName;
                    _self.pages[page.page]["title"] = newName;
                    page.file = newFileName;

                    element.querySelector(":scope > label span").innerHTML = newName;
                    element.querySelector(":scope > label a.view").setAttribute("href", page.file);
                }
            })
            .catch(error => {
                console.error(error);
                displayToast("bg-danger", "Error", "Failed to rename file.");
            });
        }
    },
